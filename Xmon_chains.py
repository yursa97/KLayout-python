...
cp3 = Contact_Pad(DPoint(-2.5e6, CHIP.dy/2), md_cpw_params, trans_in = DTrans.R270)
cp3.place(canvas)

cp4 = Contact_Pad(DPoint(2.5e6, CHIP.dy/2), md_cpw_params, trans_in = DTrans.R270)
cp4.place(canvas)
...
cp7 = Contact_Pad(DPoint(2.5e6, -CHIP.dy/2), feed_cpw_params, trans_in = DTrans.R90)
cp7.place(canvas)

cp8 = Contact_Pad(DPoint(-2.5e6, -CHIP.dy/2), feed_cpw_params, trans_in = DTrans.R90)
cp8.place(canvas)
...
turn_rad = 0.24e6
feed_segment_lenghts = [turn_rad, 1e6, 0.5e6, 2e6+cp7.end.x-cp8.end.x, 0.5e6, 1e6, turn_rad]
...
resonators_y_positions = cp8.end.y + turn_rad*3 + feed_cpw_params.b/2+res_cpw_params.b/2+resonator_offsets
...
for i in range(-(chain_length)//2, (chain_length)//2, 1):
  ...
  res = CPWResonator(res_cursor, res_cpw_params, 40e3, 6+(i+4)/10, 11.45, coupling_length=500e3,
